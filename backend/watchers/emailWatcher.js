...
function openInbox() {
  imapClient.openBox('INBOX', false, (err) => {
    ...
    // Watch for NEW emails only in real time
    imapClient.on('mail', (numNew) => {
      console.log(`[EmailWatcher] ${numNew} new email(s) — checking for payments`);
      fetchRecent();
    });

    // On startup only check last 24 hours
    fetchRecent();
  });
}

function fetchRecent() {
  // Only search emails from today — ignore all old inbox spam
  ...
  // Search only UNSEEN emails from today
