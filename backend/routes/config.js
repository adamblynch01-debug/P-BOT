...
// GET /api/config — get current config (safe public fields)
...
// POST /api/config/update — update env-style config stored in Supabase
// Bot slash commands call this
router.post('/update', async (req, res) => {
  try {
    ...
    if (!allowed_keys.includes(key.toUpperCase())) {
      return res.status(400).json({ error: `Key "${key}" is not configurable via this endpoint` });
    }

    // Store in Supabase config table
    ...
    // Update process.env live (takes effect immediately without restart)
